...
def General_Search(problem, searchMethod):
    ...
    while len(queue.data) > 0:
         ...
         for i in opened_nodes:
             if(searchMethod == SearchEnum.UNIFORM_COST_SEARCH or searchMethod == SearchEnum.GREEDY_SEARCH or searchMethod == SearchEnum.A_STAR or searchMethod == SearchEnum.HILL_CLIMBING or searchMethod == SearchEnum.BEAM_SEARCH):
                 if ((i.name in explored) or (i.name in queue.getQueueNames())):
                     opened_nodes.remove(i)
             elif(i.name in explored):
                 opened_nodes.remove(i)
         expand_queue(queue,opened_nodes,problem,searchMethod,explored)
    return False
def expand_queue(queue, nodesToAddToQueue, problem, searchMethod, explored):
    """
